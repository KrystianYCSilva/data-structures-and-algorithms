...
- NEVER overwrite or delete files generated by native CLIs (AGENTS.md, CLAUDE.md, GEMINI.md, .cursorrules, etc.); only propose append-only snippets when syncing and ask for human approval before writing.
- NEVER create root CLI-generated files (AGENTS.md, CLAUDE.md, GEMINI.md, etc.); those are created by their native CLIs.
...
- If a CLI-root file (e.g., GEMINI.md) exists and does not reference Itzamna, propose the standard append block (the Itzamna Protocol snippet) and ask the user whether to append it; do not auto-append.
