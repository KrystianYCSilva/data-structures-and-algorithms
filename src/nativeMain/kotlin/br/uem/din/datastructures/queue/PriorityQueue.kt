...
/**
 * Cria uma instância Native de PriorityQueue usando binary min-heap manual.
 */
public actual fun <T> priorityQueueOf(comparator: Comparator<T>?): MutableQueue<T> {
    return NativePriorityQueue(comparator)
}

/**
 * Implementação manual de PriorityQueue para Native.
 */
private class NativePriorityQueue<T>(private val comparator: Comparator<T>?) : MutableQueue<T> {
